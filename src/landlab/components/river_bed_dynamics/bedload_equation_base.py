...
# --------------------------------------------------------------------------- #
# Concrete wrappers  -  one class per equation
# --------------------------------------------------------------------------- #
# MPM-style wrappers (MPM, FLvB, WongAndParker, Huang) each call
# _bedload_eq_MPM_style._compute_qb() directly, passing the specific
# sub-function as an argument.  This means equation identity is determined
# entirely by which class is instantiated  -  rbd._bedload_equation is never
# re-read inside the kernel, eliminating the dual-location fragility that
# existed when all four classes delegated to the same bedload_equation()
# dispatcher.
#
# Parker1990 and WilcockAndCrowe are unchanged: they delegate to their own
# module-level bedload_equation() functions which handle fractional GSD
# internally.
...
class MPMEquation(BedloadEquation):
    ...
    *Proceedings of the 2nd IAHR Congress*, Stockholm, Sweden, 39-64.
    ...
    def calculate(self, rbd):
        """Call the MPM sub-function via the shared kernel and return results."""
...
class FLvBEquation(BedloadEquation):
    ...
    def calculate(self, rbd):
        """Call the FLvB sub-function via the shared kernel and return results."""
        from . import _bedload_eq_MPM_style as _m
...
class WongAndParkerEquation(BedloadEquation):
    ...
    *Journal of Hydraulic Engineering*, 132(11), 1159-1168.
    ...
    def calculate(self, rbd):
        """Call the Wong-Parker sub-function via the shared kernel and return results."""
        from . import _bedload_eq_MPM_style as _m
...
class HuangEquation(BedloadEquation):
    ...
    *Water Resources Research*, 46(9), W09533.
    ...
    def calculate(self, rbd):
        """Call the Huang sub-function via the shared kernel and return results."""
        from . import _bedload_eq_MPM_style as _m
...
# --------------------------------------------------------------------------- #
# Registry  -  maps user-facing string keys to equation instances
# --------------------------------------------------------------------------- #
