...
def normal_depth_from_manning(unit_discharge: float, mannings_n: float, slope: float) -> float:
...
def build_grid_and_initial_state(args: argparse.Namespace):
    ...
    if args.seed_asymmetry > 0.0:
        ...
        wake = np.exp(
            -((x - (args.cylinder_x + 2.5 * args.cylinder_diameter)) / (4.0 * args.cylinder_diameter)) ** 2
...
def make_solver(args: argparse.Namespace, grid, entry_nodes, inlet_h, inlet_u, inlet_v, exit_nodes, outlet_eta):
...
def plot_frame(grid: RasterModelGrid, args: argparse.Namespace, elapsed_time: float, output_path: Path, title: str):
    """Write a single vorticity-focused frame."""
    x = grid.x_of_node.reshape(grid.shape)
    y = grid.y_of_node.reshape(grid.shape)
    ...
    vlim = robust_symmetric_limit(omega, 98.5)

    fig, ax = plt.subplots(figsize=(12, 3.2), constrained_layout=True)
    im = ax.pcolormesh(x, y, omega, shading="auto", cmap="coolwarm", vmin=-vlim, vmax=vlim)
    ax.contour(x, y, h, levels=[args.dry_depth], colors="k", linewidths=0.7)
    ax.contour(x, y, speed, levels=8, colors="0.25", linewidths=0.35, alpha=0.5)
    ax.set_aspect("equal")
    ax.set_xlim(args.x_plot_min, args.x_plot_max)
    ax.set_ylim(0.0, args.width)
    ax.set_xlabel("x (m)")
    ax.set_ylabel("y (m)")
    ax.set_title(f"{title}, t = {elapsed_time:.1f} s")
    cbar = fig.colorbar(im, ax=ax, pad=0.01)
    cbar.set_label("vertical vorticity (1/s)")
    fig.savefig(output_path, dpi=args.dpi)
    plt.close(fig)


def plot_final_diagnostics(grid: RasterModelGrid, args: argparse.Namespace, reference: dict, output_path: Path):
    ...
    speed = calculate_speed(grid, args.dry_depth)
    omega = calculate_vorticity(grid, args.dry_depth)
    froude = speed / np.sqrt(G * np.where(h > args.dry_depth, h, np.nan))
    ...
    for ax, (arr, label, cmap, signed_limit) in zip(axes.ravel(), data):
        if signed_limit is None:
            ...
            im = ax.pcolormesh(x, y, arr, shading="auto", cmap=cmap, vmin=-signed_limit, vmax=signed_limit)
    ...
    axes[0, 1].text(
        0.02,
        0.98,
        f"hn = {reference['h_normal']:.3f} m\nUn = {reference['u_normal']:.3f} m/s\nFr = {reference['froude_normal']:.3f}",
...
def write_summary(args: argparse.Namespace, reference: dict, grid: RasterModelGrid, output_path: Path):
    ...
    with output_path.open("w") as fp:
        ...
        fp.write(f"final_depth_min_wet: {float(np.nanmin(np.where(h > args.dry_depth, h, np.nan))):.6f} m\n")
...
def run(args: argparse.Namespace) -> None:
    ...
    grid, entry_nodes, inlet_h, inlet_u, inlet_v, exit_nodes, outlet_eta, reference = build_grid_and_initial_state(args)
    flow = make_solver(args, grid, entry_nodes, inlet_h, inlet_u, inlet_v, exit_nodes, outlet_eta)

    probe_nodes = {
        "upper": nearest_node(grid, args.probe_x, args.cylinder_y + 0.75 * args.cylinder_diameter),
        "lower": nearest_node(grid, args.probe_x, args.cylinder_y - 0.75 * args.cylinder_diameter),
    ...
    print(f"Running to t = {args.total_time:.1f} s with turbulence_model={args.turbulence_model!r}")

    while flow.elapsed_time < args.total_time:
        target_time = min(args.total_time, next_plot if next_plot > flow.elapsed_time else args.total_time)
    ...
    plot_final_diagnostics(grid, args, reference, output_dir / "final_hydraulic_turbulence_fields.png")
