...
def get_nodes(grid, nrows, ncols):
    ...
    x_start, y_start = (
        grid.x_of_node[ncols - 1],
        grid.y_of_node[ncols - 1],
    )  # Upper-left
    x_end, y_end = (
        grid.x_of_node[nrows * ncols - ncols],
        grid.y_of_node[nrows * ncols - ncols],
    )  # Lower-right
    ...
    for i in range(n_samples):
        ...
        # Find closest node
        distances = np.sqrt(
            (grid.x_of_node - x_target) ** 2 + (grid.y_of_node - y_target) ** 2
        )
...
def create_combined_plot(
    ...
    fig.suptitle(
        f"{figure_title} - Time Step: {timestep}", fontsize=15, fontweight="bold"
    )
    ...
    if use_smooth_3d and SCIPY_LOCAL:
        interp = RegularGridInterpolator(
            (y1d, x1d), Z_orig, method="linear", bounds_error=False, fill_value=None
        )
    ...
    ax2.fill_between(
        x_profile, 0.0, h_profile, color="deepskyblue", alpha=0.35, zorder=2
    )
    ...
    profile_text = (
        f"center = ({center_x:.1f}, {center_y:.1f}) m, radius = {radius:.1f} m"
    )
    ax2.text(
        0.02,
        0.96,
...
def create_gif_animation(
    ...
    # Load all images
    images = []
    for _i, frame_file in enumerate(frame_files):
        try:
